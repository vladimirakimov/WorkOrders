...
{
    public class HandledUnitsEachElemGateNotEmptyValidator : PropertyValidator
    {
        public HandledUnitsEachElemGateNotEmptyValidator() : base("HandledUnits[{Index}].Gate cannot be empty.") { }
        ...
        protected override bool IsValid(PropertyValidatorContext context)
        {
            ...
            if (handledUnits.HasValue && handledUnits.Value != null && handledUnits.Value.Any())
            {
                ...
                {
                    if (handledUnit != null && string.IsNullOrWhiteSpace(handledUnit.Gate))
                    {
                        result = false;
                        context.MessageFormatter.AppendArgument("Key", nameof(handledUnit.Gate));
