...
{
    public class HandledUnitsEachElemOperantLoginNotEmptyValidator : PropertyValidator
    {
        public HandledUnitsEachElemOperantLoginNotEmptyValidator() : base("HandledUnits[{Index}].OperantLogin cannot be empty.") { }
        ...
        protected override bool IsValid(PropertyValidatorContext context)
        {
            ...
            if (handledUnits.HasValue && handledUnits.Value != null && handledUnits.Value.Any())
            {
                ...
                {
                    if (handledUnit != null && string.IsNullOrWhiteSpace(handledUnit.OperantLogin))
                    {
                        result = false;
                        context.MessageFormatter.AppendArgument("Key", nameof(handledUnit.OperantLogin));
