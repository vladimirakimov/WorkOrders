...
{
    public class RemarksEachElemTextNotEmptyValidator : PropertyValidator
    {
        public RemarksEachElemTextNotEmptyValidator() : base("Remarks[{Index}].Text cannot be null or empty.") { }
        ...
        protected override bool IsValid(PropertyValidatorContext context)
        {
            ...
            if (remarks.HasValue && remarks.Value != null && remarks.Value.Any())
            {
                ...
                {
                    if (remark != null && string.IsNullOrWhiteSpace(remark.Text))
                    {
                        result = false;
                        context.MessageFormatter.AppendArgument("Key", nameof(remark.Text));
