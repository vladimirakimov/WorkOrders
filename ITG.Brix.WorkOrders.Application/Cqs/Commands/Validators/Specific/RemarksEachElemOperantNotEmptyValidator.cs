...
{
    public class RemarksEachElemOperantNotEmptyValidator : PropertyValidator
    {
        public RemarksEachElemOperantNotEmptyValidator() : base("Remarks[{Index}].Operant cannot be null or empty.") { }
        ...
        protected override bool IsValid(PropertyValidatorContext context)
        {
            ...
            if (remarks.HasValue && remarks.Value != null && remarks.Value.Any())
            {
                ...
                {
                    if (remark != null && string.IsNullOrWhiteSpace(remark.Operant))
                    {
                        result = false;
                        context.MessageFormatter.AppendArgument("Key", nameof(remark.Operant));
